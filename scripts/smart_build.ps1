...
function Refresh-Path {
    ...
    $env:Path = (($machine, $user, 'C:\msys64\ucrt64\bin') | Where-Object { $_ } | Select-Object -Unique) -join ';'
}
...
function Find-Tool([string]$Name) {
    ...
    $candidate = "C:\msys64\ucrt64\bin\$Name.exe"
...
if (-not $compiler) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Toolchain bootstrap failed (exit code $LASTEXITCODE)."
    }

    $compiler = Find-Tool $compilerName
    if (-not $compiler) {
        throw "$compilerName is still unavailable after bootstrap."
    }
}
...
if (-not $node) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Node.js bootstrap failed (exit code $LASTEXITCODE)."
    }
    $node = Find-Tool 'node'
}

if (-not $node) {
    throw 'Node.js is unavailable, so automatic comments cannot be generated.'
}
